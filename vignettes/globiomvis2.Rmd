...
resource_files:
  - globiomvis2_files/trend_plot_example.png
---
...
It is essential that R and GAMS are linked to be able to read gdx files. See the vignette [1. Install R, packages and other relevant software](https://iiasa.github.io/globiomvis/articles/globiomvis1.html) for more information on how to do this. Here we do it manually, using the command `igdx()`.
...
trend_plot_all(df_gl = globiom, df_hs = hist, path = "c:/globiom", comb = comb_sel, file_name = "globiom_selected")
```
...
```{r echo=FALSE}
knitr::include_graphics("globiomvis2_files/trend_plot_example.png")
```
...
pdf(file = file.path("c:/globiom", paste0("globiom_trend_plots_loop_", Sys.Date(), ".pdf")))
