...
```{r eval=FALSE}
all_comb <- all_output_comb(globiom) %>%
  filter(VAR_ID %in% c("AREA", "PROD"), ITEM_AG %in% c("CORN", "CRPLND"), UNIT_ID %in% c("1000 HA", "1000 T"))

trend_plot_all(df_gl = globiom, df_hs = hist, path = "c:/test", comb = all_comb)
